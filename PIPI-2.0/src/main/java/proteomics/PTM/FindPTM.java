...
public class FindPTM {
    ...
    private final Map<String, Double> massTable;
    private Map<Integer, List<Peptide>> resultWithPtm = new HashMap<>();
    private double totalResidueMz;
    ...
    public FindPTM(Map<Integer, List<Peptide>> numResultMap, Map<Integer, SpectrumEntry> numSpectrumMap, Map<Integer, List<ThreeExpAA>> numExp3aaLists, MassTool massToolObj, Map<String, TreeSet<Integer>> siteMass100Map, double minPtmMass, double maxPtmMass, double ms1Tolerance, int ms1ToleranceUnit, double ms2Tolerance, int batchStartIdx) throws Exception {
    ...
    private PositionDeltaMassMap inferGaps(Peptide peptide, SpectrumEntry spectrumEntry, double NTermMz, double CTermMz, List<ThreeExpAA> exp3aaListSet) throws Exception {
        double expPrecursorMass = spectrumEntry.precursorMass;
        double peptideMass = peptide.getPrecursorMass();
        double deltaMass = expPrecursorMass - peptideMass;
        double[] chargeOneBMzArray = peptide.getChargeOneBIonArray();
        ...
        double totalDeltaMass = 0;

        // check N-term first
        ExpAA firstExpAa = alignedResult.get(0);
        if (Math.abs(firstExpAa.getHeadLocation() - massTable.get("PROTON")) > ms2Tolerance) {
            double expMass = firstExpAa.getHeadLocation() - massTable.get("PROTON");
            double theoMass = chargeOneBMzArray[firstExpAa.getTheoLocation() - 1] - massTable.get("PROTON");
            double massDiff = expMass - theoMass;
        ...
        if (Math.abs(totalResidueMz -lastExpAa.getTailLocation()) > ms2Tolerance) {
            double expMass =  totalResidueMz - lastExpAa.getTailLocation();
            double theoMass = chargeOneBMzArray[chargeOneBMzArray.length - 1] - chargeOneBMzArray[lastExpAa.getTheoLocation()];
            double massDiff = expMass - theoMass;
            if (Math.abs(massDiff) > ms2Tolerance) {
        ...
        for (int i = 1; i < alignedResult.size(); ++i) {
            ...
            double expMassGap = laterExpAa.getHeadLocation() - formerExpAa.getTailLocation();
            double theoMass = chargeOneBMzArray[laterExpAa.getTheoLocation() - 1] - chargeOneBMzArray[formerExpAa.getTheoLocation()];
            double massDiff = expMassGap - theoMass;
        ...
        tolerance = (double) Math.max(tolerance, 0.1);
    ...
    private PositionDeltaMassMap gaps2Ptms(Peptide peptide, PositionDeltaMassMap positionGapMap) {
        ...
        for (int i = 0; i < coordinateArray.length - 1; ++i) {
            for (int j = i + 1; j < coordinateArray.length; ++j) {
                double deltaMass1 = positionGapMap.get(coordinateArray[i]);
                double deltaMass2 = positionGapMap.get(coordinateArray[j]);
                double tolerance = 2 * ms2Tolerance;
        ...
        if (!positionGapMap.isEmpty()) {
            ...
            for (Coordinate coordinate : positionGapMap.keySet()) {
                ...
                double deltaMass = positionGapMap.get(coordinate);
                ...
                Map<Integer, Double> tempMap = pinPointPTM(siteMass100Map, aaSeq, peptideN, peptideC, proteinN, proteinC, deltaMass, 2 * ms2Tolerance);
    ...
    private Set<ThreeExpAA> cleanLocateTags(List<ThreeExpAA> inputSet, double[] peptideBIonArray, String normalizedPeptideString) {
        ...
        for (ThreeExpAA expAaList : inputSet) {
            ...
            while (idx != -1) {
                ...
                if (idx != -1) {
                    ...
                    double temp = expAaList.getTailLocation() - peptideBIonArray[idx + expAaList.size() - 1];
    ...
    private Map<Integer, Double> pinPointPTM(Map<String, TreeSet<Integer>> siteMass1000Map, String seq, boolean peptideN, boolean peptideC, boolean proteinN, boolean proteinC, double deltaMass, double tolerance) { // todo: check this function is from TagAlignment.checkJumpConstrain
        ...
        boolean ok = false;
        Map<Integer, Double> modifiedAaMap = new HashMap<>();
        ...
        for (int idx : idxArray) {
            ...
            for (int mass1000 : mass100Array) {
                if ((mass1000 >= leftBoard) && (mass1000 <= rightBoard)) {
                    modifiedAaMap.put(idx, mass1000 * 0.001);
                    ok = true;
                    break;
                }
            }
                    ...
                    for (int mass100_1 : mass100Array1) {
                        for (int mass100_2 : mass100Array2) {
                            ...
                            if ((mass100T >= leftBoard) && (mass100T <= rightBoard)) {
                                modifiedAaMap.put(idxArray[i], mass100_1 * 0.001);
                                modifiedAaMap.put(idxArray[j], mass100_2 * 0.001);
                                ok = true;
                                break;
                            }
                        }
