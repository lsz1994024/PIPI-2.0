...
public class Peptide implements Comparable<Peptide> {
    ...
    private double precursorMass = 0;
    private final double[][] ionMatrix;
    private final double[] chargeOneBIonArray;
...
//    private final String rightFlank;
    ...
    public Peptide(String peptideString, boolean isDecoy, MassTool massToolObj, int maxMs2Charge, double normalizedCrossXcorr, int globalRank) {
        ...
        precursorMass = massToolObj.calResidueMass(peptideString) + massToolObj.getMassTable().get("H2O");
...
//        this.rightFlank = rightFlank;
    ...
    public String toString() {
        if (hasVarPTM()) {
            return peptideString + "," + varPTMMap.toString();
        } else {
    ...
    public Peptide clone() {
        Peptide other = null;
        try {
            other = new Peptide(peptideString, isDecoy, massToolObj, maxMs2Charge, normalizedCrossXcorr, globalRank);
            if (varPTMMap != null) {
...
//    }
    ...
    public void setVarPTM(PositionDeltaMassMap ptmMap) {
        ...
        if (ptmMap != null) {
            for (Coordinate co : ptmMap.keySet()) {
                double deltaMass = ptmMap.get(co);
                for (int i = 0; i < ionMatrix.length / 2; ++i) {
                    double deltaMz = deltaMass / (i + 1);
            ...
            for (double deltaMass : ptmMap.values()) {
    ...
    public String getPTMContainedString(Map<Character, Double> fixModMap) { // include fix modification
        if (hasVarPTM()) {
            ...
            while (i < peptideString.length()) {
                ...
                if (!ok) {
                    double deltaMass = fixModMap.get(String.valueOf(peptideString.charAt(i)));
