...
public class TagAlignment {
    ...
    private static final double ptmPenalty = -2;

    public TagAlignment(ThreeExpAA[] sortedAasArray, Peptide peptide, Map<String, TreeSet<Integer>> siteMass100Map, double NTermMz, double CTermMz, double ms2Tolerance) {
        double[] ptmFreeBIonMz = peptide.getChargeOneBIonArray();
        ...
        for (int i = 0; i < sortedAasArray.length; ++i) {
            ...
            double expAasStartMz = expAasStart.getHeadLocation();
            ...
            if ((0 < expAasStartIdx) && (Math.abs(NTermMz - expAasStartMz) > ms2Tolerance)) {
                ...
                double theoMzGap = ptmFreeBIonMz[expAasStartIdx - 1];
                double scoreTemp;
                if (Math.abs(expAasStartMz - theoMzGap) > ms2Tolerance) {
                    ...
                    double deltaMass = expAasStartMz - theoMzGap;
                    boolean proteinN = false;
                    if (peptide.getPTMFreeSeq().contentEquals("-")) { //??
                        proteinN = true;
                    }
                    // check PTM type
                ...
                double scoreTemp = matrix[0][0].v + expAasStart.getTotalHalfIntensity();
                if (scoreTemp > score) {
                    score = scoreTemp;
                    from = new Coordinate(0, 0);
                }
            ...
            for (int iFormer = 0; iFormer < i; ++iFormer) {
                ...
                double expAasFormerEndMz = expAasFormerEnd.getTailLocation();
                if ((expAasFormerEndIdx < expAasStartIdx - 1) && (expAasFormerEndMz < expAasStartMz)) {
                    // there is a gap. but we don't know if there are PTMs in this gap.
                    double expMzGap = expAasStartMz - expAasFormerEndMz;
                    double theoMzGap = ptmFreeBIonMz[expAasStartIdx - 1] - ptmFreeBIonMz[expAasFormerEndIdx];
                    double scoreTemp;
                    if (Math.abs(expMzGap - theoMzGap) > 2 * ms2Tolerance) {
                        ...
                        double deltaMass = expMzGap - theoMzGap;
                        // check PTM type
                    ...
                    // it's a AA jump
                    double scoreTemp = matrix[iFormer + 1][expAasFormerEndIdx + 1].v + expAasStart.getTotalHalfIntensity();
                    if (scoreTemp > score) {
                        score = scoreTemp;
                        from = new Coordinate(iFormer + 1, expAasFormerEndIdx + 1);
                    }
                    ...
                    if (overlapIdx != -1) {
                        double scoreTemp = matrix[iFormer + 1][expAasFormerEndIdx - overlapIdx + 1].v + expAasStart.getTotalHalfIntensity();
                        if (scoreTemp > score) {
                            score = scoreTemp;
        ...
        for (int i = 0; i < sortedAasArray.length; ++i) {
            ...
            double expAasEndMz = expAasEnd.getTailLocation();
            int expAasEndIdx = expAasEnd.getTheoLocation();
            if ((expAasEndIdx < peptide.length() - 1) && (expAasEndMz <= CTermMz - ms2Tolerance)) {
                // there is a gap. but we don't know if there are PTMs in this gap.
                double expMzGap = CTermMz - expAasEndMz;
                double theoMzGap = ptmFreeBIonMz[ptmFreeBIonMz.length - 1] - ptmFreeBIonMz[expAasEndIdx];
                double scoreTemp;
                ...
                double scoreTemp = matrix[i + 1][expAasEndIdx + 1].v + 1;
                if (scoreTemp > score) {
                    score = scoreTemp;
                    from = new Coordinate(i + 1, expAasEndIdx + 1);
                }
    ...
    private boolean checkJumpConstrain(Map<String, TreeSet<Integer>> siteMass1000Map, String seq, boolean peptideN, boolean peptideC, boolean proteinN, boolean proteinC, double deltaMass, double tolerance) { // todo: check
