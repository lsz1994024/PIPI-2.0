...
public class CalXcorr_noUse {

    private static final Logger logger = LoggerFactory.getLogger(CalXcorr_noUse.class);
    private static final int minDecoyNum = 3000;
    private static final Double evalueTolerance1 = 3.0;
    private static final Double evalueTolerance2 = 20.0;

    private List<FinalResultEntry> finalScoredPsm = new LinkedList<>();
    private final MassTool massToolObj;

    public CalXcorr_noUse(Map<Integer, List<Peptide>> numCandidateMap, Map<Integer, SpectrumEntry> numSpectrumMap, MassTool massToolObj, BuildIndex buildIndexObj) {
    ...
    private void generateDecoyScores(FinalResultEntry psm, SparseVector expXcorrPl, TreeMap<Double, Set<String>> massPeptideMap, int gapNum, Double precursorMass, int precursorCharge) {
        // 3 Da tolerance first
        NavigableMap<Double, Set<String>> subMap = massPeptideMap.subMap(precursorMass - evalueTolerance1, true, precursorMass + evalueTolerance1, true);
    ...
    private int accumulate(FinalResultEntry psm, SparseVector expXcorrPl, NavigableMap<Double, Set<String>> subMap, int gapNum, int precursorCharge, boolean notCheckScored) {
