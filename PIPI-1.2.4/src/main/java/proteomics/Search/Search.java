...
public class Search {
    ...
    public Search(BuildIndex buildIndexObj, Map<Integer, SparseVector> numCodeMap, Map<Integer, SparseVector> numCodeMap2, Map<Integer, SparseVector> numCodeMap4, InferenceSegment inference3SegmentObj, NavigableMap<Float, List<Integer>> massNumMap, float ms1Tolerance, int ms1ToleranceUnit, float minPtmMass, float maxPtmMass, int maxMs2Charge, int batchStartIdx) {
        ...
        for (int scanNum : numCodeMap.keySet()) {
            ...
            SparseVector code2 = numCodeMap2.get(scanNum);
            SparseVector code4 = numCodeMap4.get(scanNum);
            ...
            numCodeNormSquareMap2.put(scanNum, twoNorm);
            numCodeNormSquareMap4.put(scanNum, fourNorm);
        }
            ...
            for (float scanMass : subNumMap.keySet()) {
                for (int scanNum : subNumMap.get(scanMass)) {
                    if (numCodeMap.containsKey(scanNum)) {
                        ...
                        SparseVector scanCode2 = numCodeMap2.get(scanNum);
                        SparseVector scanCode4 = numCodeMap4.get(scanNum);
                        double scanNormSquare = numCodeNormSquareMap.get(scanNum);
                        double scanNormSquare2 = numCodeNormSquareMap2.get(scanNum);
                        double scanNormSquare4 = numCodeNormSquareMap4.get(scanNum);
                        double score = 0.0;
                        double score2 = 0.0;
                        double score4 = 0.0;
                        ...
                        double temp2 = Math.sqrt(peptideCodeTwoNormSquare * scanNormSquare2);
                        double temp4 = Math.sqrt(peptideCodeFourNormSquare * scanNormSquare4);
//                        if (temp1 > 1e-6) {
                        ...
                        if (isTarget) {
                            if (Math.abs(deltaMass) <= tolerance) {
                                ...
                                if (ptmFreeResultMap.containsKey(scanNum)) {
                                    ...
                                    if (temp.size() < rankNum) {
                                        temp.add(new ResultEntry(overallscore, peptide, false, false));
                                    } else {
                                        if (overallscore > temp.peek().score) {
                                            temp.poll();
                                            temp.add(new ResultEntry(overallscore, peptide, false, false));
                                        }
                                    }
                            ...
                            if (Math.abs(deltaMass) > tolerance) {
                                ...
                                if (ptmOnlyResultMap.containsKey(scanNum)) {
                                    ...
                                    if (temp.size() < rankNum) {
                                        temp.add(new ResultEntry(overallscore, peptide, false, true));
                                    } else {
                                        if (overallscore > temp.peek().score) {
                                            temp.poll();
                                            temp.add(new ResultEntry(overallscore, peptide, false, true));
                            ...
                            if (Math.abs(deltaMass) <= tolerance) {
                                ...
                                if (ptmFreeResultMap.containsKey(scanNum)) {
                                    PriorityQueue<ResultEntry> temp = ptmFreeResultMap.get(scanNum);
                                    if (temp.size() < rankNum) {
                                        temp.add(new ResultEntry(overallscore, peptide, true, false));
                                    } else {
                                        if (overallscore > temp.peek().score) {
                                            temp.poll();
                                            temp.add(new ResultEntry(overallscore, peptide, true, false));
                            ...
                            if (Math.abs(deltaMass) > tolerance) {
                                ...
                                if (ptmOnlyResultMap.containsKey(scanNum)) {
                                    PriorityQueue<ResultEntry> temp = ptmOnlyResultMap.get(scanNum);
                                    if (temp.size() < rankNum) {
                                        temp.add(new ResultEntry(overallscore, peptide, true, true));
                                    } else {
                                        if (overallscore > temp.peek().score) {
                                            temp.poll();
                                            temp.add(new ResultEntry(overallscore, peptide, true, true));
